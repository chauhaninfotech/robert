...
$transparent_view_class = '';

$cs_header_style_class = 'cs-default-header';
if ( $cs_header_style == 'transparent-header' ) {
    
    if ( $cs_header_transparenr_view == 'transparent-fancy' ) {
        $transparent_view_class = ' cs-fancy-header';
    } elseif ( $cs_header_transparenr_view == 'transparent-modern' ) {
        $transparent_view_class = ' cs-modern-header';
    }else{
       $transparent_view_class = ''; 
    }
...
if ( is_page() ) {
    $cs_page_transparent_header_view = get_post_meta($post->ID, 'cs_page_transparent_header_view', true);
    $page_header_t_style = get_post_meta($post->ID, 'cs_page_header_style', true);
    if ( $page_header_t_style == 'transparent' ) {
        if ( $cs_page_transparent_header_view == 'fancy' ) {
            $transparent_view_class = ' cs-fancy-header';
        } elseif ( $cs_page_transparent_header_view == 'modern' ) {
            $transparent_view_class = ' cs-modern-header';
        }  else {
            $transparent_view_class = '';
        }
...
?>
<!-- Header 1 Start -->
<header class="<?php echo sanitize_html_class($cs_header_style_class);
echo ($transparent_view_class);
echo ($has_sticky);
?>" id="header">
        <?php
    ...
    ?>
    <div class="main-head">
        <div style="padding: 0px;" class="navbar navbar-default navbar-static-top container <?php echo force_balance_tags($containner_class); ?>">
                ...
                <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12 text-right">
                    ...
                    <div class="nav-right-area">
						...
						<div class="drop-down-mnu">									
							...
							<?php if ( in_array( 'cs_employer', (array) $user->roles ) ) { ?>
							<a href="<?php echo site_url().'/employee-dashboard'; ?>">Dashboard</a>
							<?php } else if ( in_array( 'um_company', (array) $user->roles ) ) { ?>
							<a href="<?php echo site_url().'/company-dashboard'; ?>">Dashboard</a>
							<?php } else if ( in_array( 'recruiter', (array) $user->roles ) ) { ?>
							<a href="<?php echo site_url().'/recruiter-dashboard/'; ?>">Dashboard</a>
							<?php } else if ( in_array( 'cs_candidate', (array) $user->roles ) ) { ?>
