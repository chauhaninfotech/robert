...
function cache_json($id, $max_tweets, $time) {
    $cache = cs_framework::plugin_dir() . 'include/cs-twitter/cache/' . $id . '.json'; //Twitter cache directory
    $cache_folder = cs_framework::plugin_dir() . 'include/cs-twitter/cache/'; //Twitter cache directory
...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
	...
	foreach ($tweets as $tweet) {
		...
		if (!empty($style)) {
			...
			case 'time_since';
			    {
				$twitter .= $timeDiff . "&nbsp;".esc_html('ago', 'cs_frame');
			    }
			    break;
			case 'ddmmyy';
			    {
	...
	$twitter .= '<li>' . esc_html__('No tweets', 'cs_frame') . '</li>';
    } //end if statement
